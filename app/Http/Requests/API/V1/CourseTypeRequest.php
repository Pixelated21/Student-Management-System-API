...
class CourseTypeRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if ($currentRouteName === self::ROUTE['store']) {
            return [
                'name' => ['bail', 'required', 'string', 'max:150', Rule::unique('course_types', 'ct_name')],
            ];
        }
        ...
        if ($currentRouteName === self::ROUTE['update']) {
            return [
                'name' => ['bail', 'required', 'string', 'max:150', Rule::unique('course_types', 'ct_name')->ignore($this->course_type)],
            ];
        }
        return [];
    }
